...
class OnceSpawner(object):
    ...
    def spawn(self, level, offset_coords):
        selected_set = self.select_spawn()
        return self.spawn_set(level, selected_set, offset_coords)
    ...
    @staticmethod
    def spawn_set(level, spawn_set, offset_coords):
        spawn_tuples = []
        spawned_objects = []
        ...
        for spawn_type, spawn_point in spawn_tuples:
            ...
            spawned_object.location.level = level
